...
import model.eszkozok.HanyoFej;
import model.eszkozok.IKotrofej;
import model.eszkozok.JegtoroFej;
import model.eszkozok.KoszoroFej;
import model.eszkozok.SarkanyFej;
import model.eszkozok.SoproFej;
import model.eszkozok.SoszoroFej;
import model.gazdasag.Arucikk;
import model.gazdasag.Bolt;
import model.gazdasag.Raktar;
import model.gazdasag.Takarito;
import model.gazdasag.Buszvezeto;
...
import model.palya.Utszakasz;
import observer.IObservable;
import observer.IObserver;

public class Vezerlo implements IObservable{
    ...
    private void tick() {
        ...
        notifyObservers(); // Szólunk a grafikának, hogy vége a körnek, lehet rajzolni!
    }
    ...
    private void beolvas(String fajlnev) throws Exception {
        ...
        // --- ÚJ GRAFIKUS BETÖLTŐ RÉSZ (Miután a fájlt végigolvasta) ---
        if (guiPanel != null) {
            guiPanel.clearViews(); // Letöröljük az előző pályát

            List<view.CsomopontView> csvLista = new ArrayList<>();
            ...
            for (Csomopont cs : csomopontok.values()) {
                double szog = 2 * Math.PI * i / (n > 0 ? n : 1);
                int x = (int) (kozeppontX + sugar * Math.cos(szog));
                int y = (int) (kozeppontY + sugar * Math.sin(szog));

                view.CsomopontView csv = new view.CsomopontView(cs, x, y);
                csvLista.add(csv);
                csomopontMap.put(cs, csv);
                guiPanel.addView(csv); // Csomópont a vászonra
                i++;
            }
            ...
            // Frissítjük a panelt, hogy megjelenjen az új város
            guiPanel.update();
        }
    }
